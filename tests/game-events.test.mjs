...
test("human bids and low Ace plays retain actor, command, legal choices and order", () => {
  ...
  const [first] = record(() => bid(g, "p0", 0, 200), "bid-0");
  assert.equal(first.player_id, "p0");
  assert.equal(first.command_id, "bid-0");
  assert.deepEqual(JSON.parse(first.payload).legalBids, [0, 1, 2, 3, 4, 5, 6]);
  const [last] = record(() => bid(g, "p1", 0, 200), "bid-1");
  ...
  const [ace] = record(() => play(g, "p0", 31, "low", 200), "play-0");
  ...
  const [second, winner] = record(() => play(g, "p1", 1, null, 200), "play-1");
  assert.equal(JSON.parse(second.payload).position, 2);
  assert.equal(winner.type, "trick_won");
  assert.equal(winner.player_id, "p1");
  tick(g, g.deadline);
  const [lead] = record(() => play(g, "p1", 2, null, 3000), "play-2");
